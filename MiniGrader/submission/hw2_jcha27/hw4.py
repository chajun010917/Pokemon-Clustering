...
def hac(features):
    ...
    for i in range (len(features)):
        a = []
        di[i] = 1
        a.append(i)           #temp[][0] contains orig. index
        a.append(features[i]) #temp[][1] contains features
        temp.append(a)
    ...
    while len(temp) > 1:
        ...
        for i in range(len(temp)):                  #Check min distance and choose two to be clustered
...
def imshow_hac(Z):
    ...
    plt.show()
'''
arr = load_data('Pokemon.csv')
arr1 = []
for i in range(0,20):
    arr1.append(calc_features(arr[i]))
linked = linkage(arr1, 'complete')
c = hac(arr1)
print(c)
print(linked)
